...
// API
import { addMedicineApi, updateMedicineApi } from "../api/medicineApi";
...
const MedicineForm = ({ type = "Add", initialData = null }) => {
  ...
  useEffect(() => {
    if (initialState) {
      setFormState({
        ...initialAddState,
        ...initialState,
        reminders: Array.isArray(initialState.reminders)
          ? initialState.reminders
          : [],
        reminderDays: Array.isArray(initialState.reminderDays)
          ? initialState.reminderDays
          : [],
      });
    }
  }, [initialState]);
  ...
  const handleDateConfirm = (date) => {
    if (activeDatePicker === "reminder") {
      ...
      if (!formState.reminders.includes(formattedTime)) {
        setFormState((p) => ({
          ...p,
          reminders: [...p.reminders, formattedTime],
        }));
      }
  ...
  // ─── BUILD PAYLOAD ─────────────────────────────────────────
  // Maps local formState shape → what the backend expects.
  // The backend model uses alias "type" for medicine_type.
  const buildPayload = () => ({
    name: formState.name.trim(),
    type: formState.type,
    dose: formState.dose?.trim() || "",
    amount: formState.amount || "1",
    reminders: formState.reminders,
    reminderDays: formState.reminderDays?.length
      ? formState.reminderDays
      : ["Daily"],
    manufactureDate: formState.manufactureDate || null,
  ...
  const handleSave = async () => {
    if (!formState.expiryDate) {
      return Alert.alert("Required", "Please set an expiry date.");
    }

    const payload = buildPayload();
    const medicineId = initialData?.id || pressedIntake?.id;
    ...
    try {
      ...
      if (type === "Add") {
        res = await addMedicineApi(payload);
      } else {
        if (!medicineId) {
          Alert.alert("Error", "Medicine ID is missing. Cannot update.");
          return;
        }
        res = await updateMedicineApi(medicineId, payload);
      }

      if (res?.success) {
        // Keep Redux in sync with server response
        dispatch({
          type: type === "Add" ? "ADD_MEDICINE" : "EDIT_MEDICINE",
          payload: res.medicine || { ...formState, ...payload },
        });
    ...
    >
      ...
      <View style={styles.card}>
        <View style={styles.rowBetween}>
          <CustomText fontWeight="bold" style={styles.sectionHeader}>
            Quick Import
          </CustomText>
          {isScanning && (
        ...
        <View style={[styles.rowBetween, { marginTop: 8 }]}>
          <View style={styles.flex1}>
            ...
            <TouchableOpacity
              ...
              onPress={() => setActiveDatePicker("mfg")}
            >
              <Icon
              ...
              <CustomText style={styles.dateText}>
                {formState.manufactureDate
                  ? new Date(formState.manufactureDate).toLocaleDateString()
                  : "Set"}
              </CustomText>
            </TouchableOpacity>
          </View>
          ...
          <View style={styles.flex1}>
            ...
            <TouchableOpacity
              style={styles.dateBtn}
              onPress={() => setActiveDatePicker("exp")}
            >
              <Icon
              ...
              <CustomText style={[styles.dateText, { color: COLORS.primary }]}>
                {formState.expiryDate
                  ? new Date(formState.expiryDate).toLocaleDateString()
                  : "Set"}
              </CustomText>
            </TouchableOpacity>
          </View>
      ...
      <View style={styles.card}>
        ...
        <View style={styles.daysContainer}>
          {MEDICINE_DAYS.map((day) => {
            const isSelected = (formState.reminderDays || []).includes(
              day.title,
            ); // ✅ FIX
            ...
            return (
              <TouchableOpacity
                ...
                onPress={() => toggleDay(day.title)} // ✅ FIX
              >
                <CustomText
        ...
        <View style={styles.timeContainer}>
          ...
          <TouchableOpacity
            ...
            onPress={() => setActiveDatePicker("reminder")}
          >
      ...
      <CustomButton
        ...
        disabled={buttonDisabled || isScanning || isSubmitting}
        onPress={handleSave}
